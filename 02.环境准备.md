...
在 cmd 输入 ipython 即可使用。ipython 提供了更好的 ui 和代码补全。

**内省**

在一个变量名的前后使用问号（？）可以显示一些关于该对象的概要，这是对象内省。使用双问好（？？）可以显示函数的源码。

问号还可以结合通配符，比如搜索`np.*load*?`。

**%run**

可以使用 %run 命令运行任意的 Python 程序文件。

如果一个Python脚本需要命令行提供参数（通过sys.argv获得），那么则需要在命令行的文件路径后面加上参数进行传递：`%run hello.py tom`

在 Jupyter notebook 中，如果你想将脚本导入一个代码单元，可以使用 %load 魔术函数

**%paste**

%paste 会获得剪贴板中的所有文本，并在命令行中作为一个代码块去执行。

%cpaste 与之类似，只不过它会给出一个特殊的提示符，让你粘贴代码，粘贴之后还可以修改。

**终端快捷键**

![](02.环境准备.assets/Image00009.jpg)

**其他魔术命令**

”命令。这些命令被设计用于简化常见任务，确保用户更容易控制 IPython 系统的行为。魔术命令的前缀符号是百分号%。

%timeit 检查一段 Python 语句的执行时间。

魔术函数也可以不加百分号%就使用，只要没有变量被定义为与魔术函数相同的名字即可。这种特性被称为自动魔术，通过 %automagic 进行启用/禁用关。

%pwd 显示当前目录。输出可以赋值给变量：`foo = %pwd`

使用 %quickref 或者 %magic 探索所有的特殊命令

%matplotlib 魔术函数可以设置 matplotlib 与I Python 命令行或 Jupyter notebook 的集成。这个命令很重要，否则你创建的图可能不会出现（notebook），或者直到结束也无法控制会话（命令行）。
